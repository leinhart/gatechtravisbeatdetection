...
{   
	...
	if(x->start == -1)
		...
		x->onsetIntensities[x->onsetCountA] = 1;//= volume;
		x->onsetCountA+=1;
		...
		if(x->oiSaveCount < x->oiSaveSize - 1)
			x->oiSaveCount += 1;
				...
				if((temp*1000.0 - x->timeStart >= x->tdOnsetsSize - 1) || x->onsetCountA > 8)
				{
					//compare to stored
					compareOnsets(x);
					//autoCorrAnalyze(x);
...
static inline void compareOnsets(t_pdbeatclassification *x){
	...
	for (i = 0; i<x->songIntervalsRSize; i+=1) {
		for (j = 0; j<9; j+=1) 
				...
				sum += pow(x->songIntervals[i][j] - x->onsetTimes[j], 2);
		}
		post("sum = %f", sum);
		sqErrors[i] = sum;
		sum = 0;
...
void *pdbeatclassification_new(void)
{
	...
	//set songIntervals
	int siTemp[3][12] = {
		{0, 456, 651, 971,  1637, 2631, 3077, 3279, 3641, -1  , -1  , -1  },  //DILIH
		{0, 446, 884, 1336, 1596, 2034, 2448, 2736, 3208, -1,-1,-1},//3644, -1  , -1  }, //VLV
		{0, 502, 752, 1246, 1514, 1756, 1990, 2422, 2596, -1,-1,-1}//2964, 3448, 3722} //Idioteque
	};
